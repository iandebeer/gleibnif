...
final case class OpenAIAgent(backend: SttpBackend[IO, Any])(using logger: Logger[IO] ):
  ...
  def extractKeywords(message: SignalSimpleMessage): EitherT[IO, ResponseException[String, Error], SignalSimpleMessage] =
    ...
    val response: IO[Response[Either[ResponseException[String, Error], OpenAIResponse]]] = request.send(backend) 
    EitherT(response.map { r => r.body match
      case Left(error) => 
