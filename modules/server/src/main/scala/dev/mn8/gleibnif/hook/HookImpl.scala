...
final case class HookImpl():
  ...
  def registerHook(request: RegisterHookRequest): IO[RegisterHookResponse] =
    ...
    parse(request.message.toString()) match
      case Left(e) => 
        log(s"error: $e")
        IO.delay(RegisterHookResponse(Some(CommonStatus(status = Status.ERROR, details = s"error: $e".asJson.noSpaces))))
      case Right(hook) => 
        log(s"hook: $hook")
        IO.delay(RegisterHookResponse(Some(CommonStatus(status = Status.OK, details = "hook registered"))))
