...
// '{"message": "Welcome to D@WNPatrol, the DIDx bot!", "number": "+27659747833", "recipients": [ "+27828870926","+27832582698" ]}
  ...
  given signalMessageDecoder: Decoder[SignalMessage] = new Decoder[SignalMessage]:
  ...
  given signalEnvelopeDecoder:Decoder[SignalEnvelope] = new Decoder[SignalEnvelope]:
      ...
      for {
        ...
        dataMessage  =
          c.downField("dataMessage").focus match
            case Some(json) => json.as[SignalDataMessage] match
       ...
       // dataMessage <- c.downField("dataMessage").as[Option[SignalDataMessage]]
        receiptMessage <- c.downField("receiptMessage").as[Option[SignalReceiptMessage]]
        syncMessage <- c.downField("syncMessage").as[Option[SignalSyncMessage]]
        sentMessage <- c.downField("sentMessage").as[Option[SignalSentMessage]]
      }
      yield SignalEnvelope(source, sourceNumber, sourceUuid, sourceName, sourceDevice, timestamp, dataMessage, receiptMessage, syncMessage, sentMessage)

  given signalDataMessageDecoder: Decoder[SignalDataMessage] = new Decoder[SignalDataMessage]:
    def apply(c: HCursor): Result[SignalDataMessage] =
      ...
      yield SignalDataMessage(timestamp, message, expiresInSeconds, viewOnce, groupInfo)

  given signalGroupInfoDecoder:Decoder[SignalGroupInfo] = new Decoder[SignalGroupInfo]:
  ...
  given signalReceiptMessageDecoder:Decoder[SignalReceiptMessage] = new Decoder[SignalReceiptMessage]:
    def apply(c: HCursor): Result[SignalReceiptMessage] =
      ...
      yield SignalReceiptMessage(when, isDelivery, isRead, isViewed, timestamps)
  
  given signalSyncMessageDecoder: Decoder[SignalSyncMessage] = new Decoder[SignalSyncMessage]:
    def apply(c: HCursor): Result[SignalSyncMessage] =
      for {
        sentMessage <- c.downField("sentMessage").as[Option[SignalSentMessage]]
        readMessages <- c.downField("readMessages").as[Option[List[SignalReadMessage]]] 
      }
      yield SignalSyncMessage(sentMessage, readMessages)
  
  given signalSentMessageDecoder: Decoder[SignalSentMessage] = new Decoder[SignalSentMessage]:
    def apply(c: HCursor): Result[SignalSentMessage] = {
      for
        ...
        viewOnce <- c.downField("viewOnce").as[Boolean]
        groupInfo <- c.downField("groupInfo").as[Option[SignalGroupInfo]]
      yield SignalSentMessage(destination, destinationNumber, destinationUuid, timestamp, message, expiresInSeconds, viewOnce, groupInfo)
    }
  
  given signalReadMessageDecoder: Decoder[SignalReadMessage] = new Decoder[SignalReadMessage]:
