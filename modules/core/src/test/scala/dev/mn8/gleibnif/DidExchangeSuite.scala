...
class DidExchangeSuite extends CatsEffectSuite {
  ...
  test("DID exchange process should complete successfully") {
    ...
    // 1. Setup test data
    val inviter = Party("did:example:123456789abcdefghi", Role.Inviter, State.InvitationSent)
    val invitee = Party("did:example:987654321hgfedcba", Role.Invitee, State.RequestReceived)

    // 2. Run the DID exchange process
    // Note: replace `runDidExchange` with the actual method in your code
    val result: IO[Either[Error, State]] = new DidExchange().runDidExchange(inviter, invitee)
