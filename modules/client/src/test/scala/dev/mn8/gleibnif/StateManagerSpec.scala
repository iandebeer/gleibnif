...
class StateManagerTests extends CatsEffectSuite {
  ...
  test("StateManager should correctly handle concurrent updates") {
    ...
    def updates(sm: StateManager) = (1 to 100).toList.map(i => update(sm, i)).parSequenceN(100)
    ...
    val state =  for 
      ...
      _ <- updates(sm)
      s <- sm.getState("1")
      _ <- IO.println(s"after concurrent updates state = ${s.getOrElse("None")}")

    yield (s) 
    IO.println(s"after concurrent updates state = ${state.map(s => s.getOrElse("None"))}")
